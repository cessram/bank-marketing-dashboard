...
To run: streamlit run bank_marketing_dashboard.py
"""
...
if page == "🏠 Home":
    ...
    st.markdown('<p class="sub-header">Predicting Term Deposit Subscription Using Machine Learning</p>', unsafe_allow_html=True)
...
# ============================================================================
    ...
    if viz_option == "Target Distribution":
        ...
        **What we're analyzing:** Which professions have highest conversion rates to inform 
        ...
        colors = [COLORS['success'] if x > 15 else COLORS['warning'] if x > 10 else COLORS['danger'] for x in job_stats.values]
        ...
        **What we're analyzing:** Temporal patterns in subscription behavior across all twelve months,
        ...
        fig = make_subplots(rows=1, cols=2, subplot_titles=('Monthly Subscription Rate Trends', 'Campaign Volume by Month'))
...
# ============================================================================
    ...
    if st.button("🔮 Predict Subscription Likelihood", type="primary"):
        ...
        if duration < 300:
            recommendations.append("📞 **Increase call engagement** - Longer calls (>5 min) correlate with 2.43x higher conversion")
        if campaign > 3:
            recommendations.append("🚫 **Reduce contact frequency** - Over-contacting hurts conversion; limit to 3-4 attempts")
        if poutcome == 'unknown':
        ...
        if job in ['blue-collar', 'entrepreneur']:
            recommendations.append(f"⚠️ **Lower-potential segment** - {job.capitalize()}s historically show below-average interest")
